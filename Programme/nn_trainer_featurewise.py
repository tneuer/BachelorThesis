...
several_nn = [[100, 100, 50]]
activations_list = [["elu"]*3]
DropOut_list = [None]
...
folder = "../allOutputs/FeaturewiseStopping/"
######################
...
def train_neural_network(x):
    ...
    "features": used_features, "train_examples": nr_examples, "time": t, \
...
if __name__ == "__main__":
    ...
    for nodes in several_nn:
        ...
        print("Read data...")
        with h5py.File("../data/Processed_Train_mu_400000.h5", "r") as Data:
            dataTotal = Data["Train"][:nr_examples]
        if PCA:
            print("PCA data...")
            dataTotal = inc_pca.transform(dataTotal)

        print("Read labels...")
        with open("../data/Train_mu_labels_400000.pickle", "rb") as f:
        ...
        with open("../allOutputs/XGB/XGBImportantFeatures.txt") as f:
            lines = f.readlines()
            features = []
            for line in lines:
                pattern = "[0-9]+\).*$"
                ...
                if found:
                    pattern = " .*"
                    found = re.findall(pattern, found[0])
                    features.append(found[0][1:])
                else:
                    features.append(0)
        ...
        for feature in features:
            if feature == 0:
                if len(used_features) > 14:
                    nr_features = len(used_features)
                    x = tf.placeholder('float', [None, nr_features], name="x")
                    y = tf.placeholder('float', name="y")
                    tr_phase = tf.placeholder(tf.bool, name="trainPhase")
                    print(used_features)
                    used_col_nrs = np.array([fnames[f] for f in used_features])
                    data = dataTotal[:, used_col_nrs]
                    test_x = test_x_total[:, used_col_nrs]
                    train_neural_network(x)
            else:
                if not feature == "nTrack":
                    used_features.append(feature)
                else:
                    used_features.append("nTrackN")
