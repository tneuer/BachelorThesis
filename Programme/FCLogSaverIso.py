...
data = data[["muminus_PT", "muminus_TrEta", "muplus_PT", "muplus_TrEta", "Max_Iso", "Max_MINIP"]]
data = data.values
    ...
    for folder,statsNr in zip(folders, stats_nr):
        ...
        with open(folder+"/Models/checkpoint", "r") as f, tf.Session() as sess:
            ...
            for line in lines[1:]:
                ...
                filename = folder+"/Models/" + regExp.group(0)[:-1]
                ...
                try:
                    pred = pred_proba.eval({x: test_x, tr_phase: 0})
                except ValueError:
                    pred = pred_proba.eval({x: test_x[:, :-1], tr_phase: 0})
...
def build_figures(stats, nr, path = None, histogram = "Z0_MM", acc=False, lastEpoch=False):
    ...
    with open(folder+"/Models/checkpoint", "r") as f, tf.Session() as sess:
        ...
        for line in lines[1:]:
            ...
            pred1 = [np.argmax(p) for p in pred]
