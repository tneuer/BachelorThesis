...
several_nn = [[6, 4, 4]]#, [6, 3], [4], [6,6,5,5,4,4,3,3,2], [4,3,2], [3]]
activations_list = [["elu"]*len(net) for net in several_nn]# + [["tanh"]*len(net) for net in several_nn] + [["relu"]*len(net) for net in several_nn]
# several_nn *= 3
DropOut_list = [None]
...
nr_examples_list = [10000]
earlyStopping = 20

BN_list = [False]
PCA = False

folder = "../TESTISO/IsolationIP/"
datafolder = "../data/"
######################
...
isolationFeatures = ["muminus_PT", "muminus_TrEta", "muminus_cpt_0.1", "muminus_cpt_0.5", "muminus_iso", "muminus_high_iso", "muminus_MINIP", "muminus_high_IP", "muplus_PT", "muplus_TrEta", "muplus_cpt_0.1", "muplus_cpt_0.5", "muplus_iso", "muplus_high_iso", "muplus_MINIP", "muplus_high_IP", "choose_muon_iso", "choose_muon_MINIP"]

iso_stand_features = isolationFeatures[:5] + ["muminus_MINIP"] + isolationFeatures[8:13] + ["muplus_MINIP"]
iso_log_features = ["muminus_PT", "muminus_cpt_0.1", "muminus_cpt_0.5", "muminus_iso", "muminus_MINIP", "muplus_PT", "muplus_cpt_0.1", "muplus_cpt_0.5", "muplus_iso", "muplus_MINIP"]

# Load test data
print("Read test...")
with h5py.File("{}Processed_Iso_Val_mu_263093.h5".format(datafolder), "r") as valData:
    test_x = valData["Val"][:]

test_x = pd.DataFrame(test_x, columns = iso_features)
for col in test_x.columns.values:
    print(test_x[col][:20])
test_x = test_x[["muminus_PT", "muminus_TrEta", "muminus_iso", "muminus_MINIP", "muplus_PT", "muplus_TrEta", "muplus_iso", "muplus_MINIP"]]

test_x["Max_Iso"] = test_x[["muminus_iso", "muplus_iso"]].max(axis=1)
test_x["Max_MINIP"] = test_x[["muminus_MINIP", "muplus_MINIP"]].max(axis=1)

test_x = test_x[["muminus_PT", "muminus_TrEta", "muplus_PT", "muplus_TrEta", "Max_Iso"]]

test_x = test_x.values

with open("{}Val_Iso_mu_labels_263093.pickle".format(datafolder), "rb") as f:
...
def train_neural_network(x):
    ...
    with tf.name_scope("Model"):
        if network_type == "FC":
            prediction = feed_forward_nn(x, nr_features, nr_classes, nodes, activations, DropOut, bn, tr_phase)
            ...
            saveDirectory = "{}FC/".format(folder)
            ...
            replicate = tf.identity(prediction, name="replicate")
            cost = tf.reduce_mean(tf.pow(x - prediction, 2), name="cost")
            saveDirectory = "{}AutoEncoder/".format(folder)
    ...
    with tf.Session() as sess: #config=tf.ConfigProto(log_device_placement=True)
        ...
        for epoch in range(hm_epochs):
            ...
            if (epoch % 40 == 0 or epoch == hm_epochs-1):
                print("Model saved.")
            ...
            print()
            prdct = pred_proba.eval(feed_dict = {x: test_x, y: test_y, tr_phase: 0})
            prdct1 = [np.argmax(p) for p in prdct]
            print(np.array(prdct))
            tst_y = [np.argmax(y) for y in test_y]
            print(te_acc, tr_acc, accuracy_score(tst_y, prdct1), roc_auc_score(tst_y, prdct[:, 1]))
            print()
    ...
    "batch": batch_size, "epochs": len(losses[0]), "activations": activations, "optimizer": optimizer_name, \
    "features": isolationFeatures, "train_examples": nr_examples, "time": t, \
    "test_size": test_x.shape[0], "DropOut": DropOut, "NNType": network_type, \
    "feature_size": nr_features, "fcost": fcost, \
    "standFeatures": iso_stand_features, \
    "LogTransformed": iso_log_features, \
...
if __name__ == "__main__":
    ...
    for nodes in several_nn:
        ...
        print("Read data...")
        with h5py.File("{}Processed_Iso_Train_mu_2367829.h5".format(datafolder), "r") as Data:
        ...
        data = data[["muminus_PT", "muminus_TrEta", "muplus_PT", "muplus_TrEta", "Max_Iso"]]

        isolationFeatures = data.columns.values
        data = data.values

        print("Read labels...")
        with open("{}Train_Iso_mu_labels_2367829.pickle".format(datafolder), "rb") as f:
